...
"Convert a class name CLASS and semantic TAG to a jde-usages signature type."
  (let ((ttype  (semantic-tag-type tag))
        ...
        (tmodifiers (semantic-tag-get-attribute tag :typemodifiers))
        )
    (list tclass
          class
          (if (or (equal tname (jde-parse-get-unqualified-name class))
                  (jde-usages-ends-with class (concat "$" tname)) ;; inner class constructors
                  )
              "<init>"
            tname)
          ...
          (cond
           ((eq tclass 'function) (mapcar (lambda (arg)
...
(let ((properties `(list
                    ...
                    :format "%[%t%]\n"
                    :notify (lambda (widget child &optional event) ,notify-expr ))))
...
This function uses `jde-usages-find-class-source to get
  ...
  (let* ((class-name (or class-name (nth 1 sig))) ;; 
    ...
    (if (jde-usages-find-class-source class-name t)
     ...
     (let ((tags (semantic-find-tags-by-name source-method-name (semantic-tag-type-members (semantic-current-tag-of-class 'type)))))
       (cond
        ...
        ((and (eq 1 (length tags)) (eq (semantic-tag-class (car tags)) 'function))
         (semantic-go-to-tag (car tags))
         (jde-usages-momentarily-highlight-tag (car tags)))
